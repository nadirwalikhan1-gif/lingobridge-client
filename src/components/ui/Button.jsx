...
export default function Button({
  ...
  loading = false, disabled = false, tooltip,
  onClick, type = 'button', className = '', ...props
}) {
  const base = 'inline-flex items-center justify-center gap-1.5 font-semibold rounded-[10px] transition-all duration-150 select-none focus:outline-none focus:ring-2 focus:ring-offset-1';

  const variants = {
    primary:   'bg-lb-primary text-white hover:bg-lb-deep focus:ring-lb-primary/30 shadow-sm hover:shadow-md active:scale-[0.98]',
    secondary: 'bg-white text-lb-ink border border-lb-border hover:bg-lb-surface hover:border-lb-primary/40 focus:ring-lb-primary/20 shadow-sm',
  ...
  return (
    <button
      ...
      className={`${base} ${variants[variant] || variants.primary} ${sizes[size] || sizes.md} ${isDisabled ? 'opacity-40 cursor-not-allowed pointer-events-none' : ''} ${className}`}
      {...props}
    >
      {loading && (
