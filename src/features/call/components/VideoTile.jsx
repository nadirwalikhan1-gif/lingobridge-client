    ...
    <div className="relative w-full h-full rounded-xl overflow-hidden bg-[#1a1a2e]">
      ...
      {label && (
        <div className="absolute bottom-2 left-2 right-2 flex items-center justify-between">
          <div className="flex items-center gap-1.5 px-2 py-0.5 rounded bg-black/50">
            <span className="text-white text-xs font-medium">
              {label}{isLocal ? ' (you)' : ''}
            </span>
            {muted && (
              <svg className="w-3 h-3 text-[#F09595]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
          ...
          {/* Camera off badge */}
          {camOff && (
            <div className="px-1.5 py-0.5 rounded bg-black/50">
              <svg className="w-3 h-3 text-[#F09595]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
