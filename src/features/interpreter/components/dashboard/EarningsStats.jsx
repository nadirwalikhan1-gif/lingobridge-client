...
// FIXES APPLIED: 🟠 Acceptance rate stat card, 🟡 Monthly certification hours
...
export default function EarningsStats({ stats = {} }) {
  const {
    ...
    // 🟠 New props
    acceptanceRate = '—', acceptanceTrend = null,
    // 🟡 New props
    certHours = '—', certTarget = '40h',
  } = stats

  // Calculate certification progress percentage
  const certProgress = (() => {
    if (!certHours || certHours === '—' || !certTarget) return 0
    const hours = parseFloat(certHours.replace(/[^0-9.]/g, '')) || 0
    const target = parseFloat(certTarget.replace(/[^0-9.]/g, '')) || 40
  ...
  return (
    <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
      ...
      {/* 🟠 Acceptance rate — full-width row below on mobile, 4th col on lg */}
      <div className="col-span-2 lg:col-span-1 rounded-xl px-4 py-4 bg-[#E1F5EE] border border-[#CECBF6] flex flex-col justify-between min-h-[110px]">
        <div className="flex items-center justify-between">
          ...
          <span className="text-[10px] font-semibold px-1.5 py-0.5 rounded bg-[#0F6E56] text-white">{acceptanceRate}</span>
        </div>
        <div className="mt-2">
          <p className="text-[28px] font-semibold leading-none text-[#0F6E56]">{acceptanceRate}</p>
          {acceptanceTrend && (
      ...
      {/* 🟡 Certification hours — full-width row below on mobile, 4th col on lg */}
      <div className="col-span-2 lg:col-span-1 rounded-xl px-4 py-4 bg-[#FAEEDA] border border-[#F5D0A9] flex flex-col justify-between min-h-[110px]">
        <div className="flex items-center justify-between">
          ...
          <span className="text-[10px] font-semibold px-1.5 py-0.5 rounded bg-[#854F0B] text-white">{certHours}</span>
        </div>
        <div className="mt-2">
          <p className="text-[28px] font-semibold leading-none text-[#854F0B]">{certHours}</p>
          <div className="mt-2">
            ...
            <div className="w-full h-1.5 bg-[#F5D0A9] rounded-full overflow-hidden">
              <div className="h-full bg-[#BA7517] rounded-full transition-all" style={{ width: `${certProgress}%` }} />
            </div>
          </div>
        </div>
      </div>
