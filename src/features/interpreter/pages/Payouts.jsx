    ...
    <div className="space-y-3 max-w-2xl">
      ...
      <div className="lb-card !bg-[#26215C] text-white">
        ...
        <div className="flex gap-2">
          <button
            ...
            disabled={withdrawing || balance == null || balance < 50}
      ...
      <div className="lb-card">
        ...
        {loading ? (
          ...
          <p className="text-[13px] text-lb-muted text-center py-6">No payouts yet.</p>
        ) : (
            ...
            {payouts.map((p) => (
              <div key={p.id} className="flex items-center gap-3 py-2.5 first:pt-0 last:pb-0">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-[13px] font-medium text-lb-ink">{p.method ?? p.paymentMethod ?? 'Bank transfer'}</p>
                  <p className="text-[11px] text-lb-muted">
                    {p.account ?? p.accountLast4 ? `**** ${p.accountLast4}` : ''}
                    {(p.account || p.accountLast4) && ' · '}
                ...
                <div className="text-right shrink-0">
                  ...
                  <span className={`text-[10px] font-medium px-1.5 py-0.5 rounded ${
                    p.status === 'completed' ? 'bg-[#EAF3DE] text-[#3B6D11]' :
                    p.status === 'pending'   ? 'bg-[#FAEEDA] text-[#854F0B]' :
